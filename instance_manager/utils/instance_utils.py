...
def check_user_limit_before_create(user_type):
	"""Check if a new user can be created based on user type"""
	settings = InstanceSettings.get_instance_settings()
	...
	if count >= limit:
		return {
			"allowed": False,
			"reason": f"{type_name} user limit ({limit}) reached"
		}
...
def get_expiry_warning():
	"""Get the expiry warning if applicable"""
	settings = InstanceSettings.get_instance_settings()
	...
	if settings.is_expired():
		return {
			"status": "expired",
			"message": f"License expired on {settings.expiry_date}"
		}
...
def validate_instance_active():
	"""Validate that the instance is active and not expired"""
	settings = InstanceSettings.get_instance_settings()
...
def get_dashboard_status():
	"""Get dashboard status for homepage cards"""
	settings = InstanceSettings.get_instance_settings()
